...
```Linux
[linjing@CentOS_Hadoop_01 hadoop-3.2.1]$ mkdir input   //1. 创建在 hadoop-3.2.1 文件下面创建一个 input 文件夹
[linjing@CentOS_Hadoop_01 hadoop-3.2.1]$ cp etc/hadoop/*.xml input   //2. 将 Hadoop 的 xml 配置文件复制到 input
[linjing@CentOS_Hadoop_01 hadoop-3.2.1]$ bin/hadoop jar share/hadoop/mapreduce/hadoop-mapreduce-examples-3.2.1.jar grep input output 'dfs[a-z.]+'   //3. 执行 share 目录下的 MapReduce 程序。output 不能预先创建，输出符合正则表达式的个数
[linjing@CentOS_Hadoop_01 hadoop-3.2.1]$ cat output/*  //4. 查看输出结果
```
...
4.回到 Hadoop目录/opt/module/hadoop-3.2.1
5.执行程序
